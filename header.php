<!DOCTYPE HTML><!--[if lt IE 9]><html class="ie"><![endif]-->
<!--[if !IE]><html><![endif]-->
<html>
	<head>
     <link href='http://fonts.googleapis.com/css?family=Muli:300,400,300italic,400italic' rel='stylesheet' type='text/css'>

     
		<meta charset="UTF-8">
		<title>Rick Pas | Contemporary Realism</title>
		
		<!-- Viewport meta tag to prevent iPhone from scaling our page -->
		<meta name="viewport" content="width=device-width, initial-scale=1"/>
		
		<link rel="stylesheet" href="./css/style.css"/>
       
<!--[if IE 8]><link href="css/ie8.css" rel="stylesheet" type="text/css" />
<![endif]-->
			
			<!-- Normalize hide address bar for iOS and Android -->
		<script src="./js/hideaddressbar.js"></script>
	
    
		
		<!-- Using picturefill to load conditional images and matchmedia to provide support for IE8 and below + older browsers -->
        <script type='text/javascript' src="http://ajax.googleapis.com/ajax/libs/jquery/1.8.3/jquery.min.js"></script>
		<script src="js/picturefill.js"></script>
		<script type="text/javascript" src="js/function10_11.js"></script>
		<script src="js/matchmedia.js"></script>
		<script type='text/javascript' src='js/modernizr.custom.js'></script>
        <!--<script type="text/javascript" src="js/modernizr.custom.58565.js"></script>-->
		<script type='text/javascript' src='js/imagesloaded.pkgd.min.js'></script>
		<script type='text/javascript' src='js/grayscale.js'></script>       
		<script type="text/javascript" src="js/notouch.js"></script>
        
<!--[if lt IE 9]>
<script src="//ajax.googleapis.com/ajax/libs/jquery/1.10.0/jquery.min.js"></script>
<![endif]-->

<!--[if IE 8]>
<script src="./js/checkbox.js"></script>
<![endif]-->

     <!--[if IE 6]>
<script src="./js/selectivizr-min.js"></script>
<![endif]-->
   
        
		<!--[if lt IE 9]>
			<script src="//html5shim.googlecode.com/svn/trunk/html5.js"></script>
		<![endif]-->
			

	</head>

	<body lang="en"> 
		
		<div id="container-header">
			<header id="masthead" class="wrapper">
				<h2>Rick Pas</h2>
				<h1>Contemporary Realism</h1>
			</header>
		</div>
        <div id="icon">
        	<a href="https://www.facebook.com/pages/Rick-Pas-Studio/316205728224?ref=hl." target="_blank"><img class="faceicon" height="21" width="21" alt="facebook" src="img/facebook.png"></a>&emsp;
			<a href="https://www.pinterest.com/rickpas/" target="_blank"><img class="faceicon" height="21" width="21" alt="pinterest" src="img/pinterest.png"></a>&emsp;
			<a href="https://www.instagram.com/?hl=en" target="_blank"><img class="faceicon" height="21" width="21" alt="instagram" src="img/instagram.png"></a>&emsp;
			<a href="mailto:rickpas@rickpas.com"><img class="faceicon" height="22" width="21" alt="email to rick" src="img/email.png"></a>
		</div>
		<div id="container-nav homeimages">
			<section id="nav" class="wrapper">
                  <!-- Checkbox Hack markup -->
        		<input type="checkbox" id="toggle" />
        		<label for="toggle" onclick>Menu</label>
        		<!-- /Checkbox Hack markup -->
				 <nav>
		            <ul>
		                <li><a href="index.php">Home</a></li>
						<li><a href="bio.php">Biography</a></li>
						<li><a href="contact.php">Contact</a></li>
		            </ul>
            
                  </nav>
			</section>
		</div>